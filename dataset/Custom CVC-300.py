...
_root = "/".join(__file__.split("/")[:-1]) + "/source/CVC-ClinicDB"

print(os.listdir(_root))

class CustomCVC_300(Dataset):
    def __init__(self, root:str = _root, x='train', args:argparse = None):
        ...
        self._images = glob.glob(self.root+ "/images/*")
        self._segs = glob.glob(self.root+ "/masks/*")
    ...
    def __getitem__(self, idx):
        ...
        torch_mask = torch.from_numpy(transformed_mask).squeeze(-1).permute(-1, 0, 1).float()
